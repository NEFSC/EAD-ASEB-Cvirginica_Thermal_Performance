...
-   Last updates: oct, 31, 2024

# Thermal Performacne (pilot test) & INFISH projet with Indya Limpkin & Ashley Hallengren

## Ran a Succinate kit on C virginica gill tissue homogenate, corrected to
...
# plate 1 
Succ_Plate1_nodilution <- Spec_to_table('Data/Succinate/20241010_Succinate_Plate1_nodilution.txt', 1, 20241010) %>% dplyr::mutate(About = 'no dilution')
Succ_Plate1_Post5xdilution <- Spec_to_table('Data/Succinate/20241010_Succinate_Plate1_5x_dilution_POST.txt', 1, 20241010) %>% dplyr::mutate(About = '5x dilution post')
Succ_Plate1_Post5xdilutionPost_30mins <- Spec_to_table('Data/Succinate/20241010_Succinate_Plate1_nodilution_5x_dilution_POST_30MIN.txt', 1, 20241010) %>% dplyr::mutate(About = '5x dilution post 30 mins')

Succ_Plate1_master <- do.call("rbind", list(Succ_Plate1_nodilution, Succ_Plate1_Post5xdilution, Succ_Plate1_Post5xdilutionPost_30mins))

# plate 2
Succ_Plate2_5xdilutionPre <- Spec_to_table('Data/Succinate/20241010_Succinate_Plate2_5x_dilution_PRE.txt', 2, 20241010)  %>% dplyr::mutate(About = '5x dilution pre')
...
# plate 1 
TP_Plate1_master <- Spec_to_table('Data/Total_Protein_Succinate/20241011_TotalProtein_Succinate_Plate1.txt', 1, 20241011)  %>%  dplyr::rename(Abs_562nm = Abs_412nm)

# plate 2
TP_Plate2_master <- Spec_to_table('Data/Total_Protein_Succinate/20241011_TotalProtein_Succinate_Plate2.txt', 2, 20241011)   %>%  dplyr::rename(Abs_562nm = Abs_412nm)
...
metadata <- read.csv('Data/Succinate_metadata.csv', sep = ",", header=T)

# rename a few columns so they can merge with the data (in next chunk)

# Plate 1 
metadata.Succ_Plate_1 <- metadata %>% 
                        dplyr::filter(Assay.Type %in% "Succinate" & Plate.Number == 1) %>% 
                        dplyr::rename(Plate = Plate.Number,
                                      well  = Well.ID)
# Plate 2
metadata.Succ_Plate_2 <- metadata %>% 
                        dplyr::filter(Assay.Type %in% "Succinate" & Plate.Number == 2) %>% 
                        dplyr::rename(Plate = Plate.Number,
                                      well  = Well.ID)
...
# Succinate
# Plate 1 
Plate1 <- merge(metadata.Succ_Plate_1, Succ_Plate1_master, by = c('Plate', 'well'))                          
nrow(Succ_Plate1_master) == nrow(Plate1) # must be TRUE
Plate1 <- subset(Plate1, !is.na(Sample.Type)) # now 288  total rows with NAs omitted (not sample, blank nor stnadard)


# Plate 2
Plate2 <- merge(metadata.Succ_Plate_2, Succ_Plate2_5xdilutionPre, by = c('Plate', 'well'))                          
nrow(Succ_Plate2_5xdilutionPre) == nrow(Plate2) # must be TRUE - NOTE THIS IS FALSE BECAUSE THE METADATA DID NOT INCLUDE NON SAMPLE STANDARD OR BLANK WELLS, OKAY
Plate2 <- subset(Plate2, !is.na(Sample.Type)) # METADATA WAS 58 AND NOW PLATE IS 58  - this confirms that the metadata just didnt have the empty well IDs included
...
nrow(TP_Plate2_master) == nrow(Plate2.TP) # must be TRUE - NOT HERE THOUGH again the metadtata did not include the empty IDs, we have 78 with sample, all good
Plate2.TP <- subset(Plate2.TP, !is.na(Sample.Type)) # now 78 total rows with NAs omitted (not sample, blank nor stnadard)
...
# Citrate synthase 
# Plate 1 
Plate1.samples   <- Plate1 %>% dplyr::filter(Sample.Type %in% 'Sample') %>% dplyr::rename(Abs_570nm = Abs_412nm)
# Plate1.blanks    <- Plate1 %>% dplyr::filter(Sample.Type %in% 'Blank') # space, why... whatevs
Plate1.standards <- Plate1 %>% dplyr::filter(Sample.Type %in% c('Blank','Standard')) %>% dplyr::rename(Abs_570nm = Abs_412nm)
...
# Plate 2
Plate2.samples   <- Plate2 %>% dplyr::filter(Sample.Type %in% 'Sample') %>% dplyr::rename(Abs_570nm = Abs_412nm)
# Plate2.blanks    <- Plate2 %>% dplyr::filter(Sample.Type %in% 'Blank')  # space, why... whatevs
Plate2.standards <- Plate2 %>% dplyr::filter(Sample.Type %in% c('Blank','Standard')) %>% dplyr::rename(Abs_570nm = Abs_412nm)
...
###  THIS IS WHERE I LEFT OFF ON 11/7/2024 
* NOTE: the code below is from the citrate synthase script, need to change throughout to accomodate the variables and values in chunks above


## Succinate Calculations
...
# output 
pdf(paste("Output/Colorimetric_assays/Succinate/Standard_Curve_Succinate.pdf", sep =''), 
    width=10, 
    height=5)
